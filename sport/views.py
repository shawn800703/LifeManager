...
def record(request,id = 0):
    ...
    sports = SportItem.objects.filter(state='1')
    datas = SportRecord.objects.filter(cuser=userid)
...
def chartData(request,s_date,e_date):
    ...
    count = SportRecord.objects.filter(cuser=userid, sdate__range=(s_date, e_date)).values('sportid').annotate(dcount=Count('sportid')).values('sportid__name','dcount')
   
    # count = SportRecord.objects.filter(cuser=userid).values('sportid').annotate(dcount=Count('sportid')).values('sportid__name','dcount')
    count = json.dumps(list(count))
